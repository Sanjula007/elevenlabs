...
export default function TextToSpeechPage() {
    ...
    const handlePause = useCallback(async (seconds) => {
        async function convertToBlobUrl(fileUrl) {
        ...
        const bloburl =  await convertToBlobUrl('http://localhost:3000/silent_1-second.wav')
      //  const audioBase64 = await fetchAudioAsBase64Blob('http://localhost:3000/silent_1-second.wav');
        const pendingSpeech: GeneratedSpeech = {
            id: nanoid(),
            text: `Paused at ${seconds} seconds`,
            audioBase64: bloburl,
            createdAt: new Date(),
            status: 'complete',
        };
    ...
    const handleCombineAndDownload = useCallback(async () => {
        ...
        const completedSpeeches = speeches.filter((speech) => speech.status === 'complete');
